...
__show_description_of_this_command() {
    echo "${COLOR_GREEN}autogen.sh ($MY_VERSION) is a POSIX shell script which is designed to manage GNU Autotools(autoconf automake) and other softwares used by this project.\n${COLOR_OFF}"
}

__show_help_of_this_command() {
    __show_description_of_this_command
    cat <<EOF
Usage:
./autogen.sh -h | --help             -- show help of this command.

./autogen.sh -V | --version          -- show version of this command.

./autogen.sh env                     -- show current machine os info and environment varables

./autogen.sh upgrade [-x | --china]  -- upgrade this software.

./autogen.sh integrate zsh [-x | --china]           -- download a zsh-completions script to a approprivate location.

./autogen.sh configure [ -x | -d | -rc-file=FILE ]  -- generate the configure shell script.
EOF
}
...
main() {
    ...
    do
        case $1 in
            ...
            --rc-file=*)
                ...
                if [ -z "$RC_FILE" ] ; then
                    die "--rc-file=FILE FILE must not be empty."
                else
                    die_if_file_is_not_exist "$RC_FILE"
                fi
                ;;
    ...
    regist_dependency required exe m4
    ...
    if [ "$NATIVE_OS_TYPE" = 'openbsd' ] ; then
        ...
        regist_dependency required exe autoconf-$AUTOCONF_VERSION ge "$AUTOCONF_VERSION_MREQUIRED"
        regist_dependency required exe automake-$AUTOMAKE_VERSION
    fi

    regist_dependency required exe autoconf ge "$AUTOCONF_VERSION_MREQUIRED"
    regist_dependency required exe automake
    regist_dependency required exe make:gmake:bmake
    ...
    if __is_libtool_used ; then
        regist_dependency required exe libtoolize
    fi
